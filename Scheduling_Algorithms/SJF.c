...
void sjf(Process p[], int n) {
  ...
  while (completed < n) {
    ...
    // the heap is min-heap sow the one with lowest execution time is placed on
    // the start and offcurse the other are ordred
    for (int i = 0; i < n; i++) {
    ...
    // same logic on fcfs and all other algorithms if the quur is empty then the
    // cou is idle
